...
export default function ClubDetail() {
  ...
  const { data: members = [] } = useQuery({
    ...
    queryFn: () => base44.entities.BookClubMember.filter({ club_id: id }),
    enabled: !!id,
  });

  const { data: reviews = [] } = useQuery({
    queryKey: ['club-reviews', id],
    queryFn: () => base44.entities.BookReview.filter({ club_id: id }),
    enabled: !!id,
  });
  ...
  return (
    <div>
      <AppHeader title={club.name} />
      <div className="p-6 md:p-10 max-w-2xl mx-auto space-y-6">
        <div>
          <h1 className="font-heading text-2xl font-bold hidden md:block">{club.name}</h1>
